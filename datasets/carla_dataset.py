...
class CarlaDataset(Dataset):
    ...
    def points_to_voxels(self, voxel_grid, points, t_i):
        ...
        voxels = np.floor((points - self.min_bound) / self.voxel_sizes).astype(int)
    ...
    def __getitem__(self, idx):
        ...
        if self.voxelize_input:
            voxel_input = np.zeros(
                (idx_range.shape[0], int(self.grid_dims[0]), int(self.grid_dims[1]), int(self.grid_dims[2])),
                dtype=np.float32)
        t_i = 0
        ...
        if self.get_gt:
            output = np.fromfile(self._eval_labels[idx_range[-1]], dtype=np.uint32).reshape(self._eval_size).astype(
                np.uint8)
